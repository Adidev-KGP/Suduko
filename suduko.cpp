...
bool check(int i,int j,int val){
	for(int k=0;k<9;k++){
		if(k!=j && val==board[i][k])return 0;
		if(k!=i && val==board[k][j])return 0;
	}
	int basex = (i/3)*3;
	int basey = (j/3)*3;
	for(int delx = 0;delx<3;delx++){
		for(int dely = 0;dely<3;dely++){
			if(make_pair(basex+delx,basey+dely)!=make_pair(i,j) && val==board[basex+delx][basey+dely]){
				return 0;
			}
		}
	}
